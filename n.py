...
_google_api_key: str = "" # Initialize with empty string

try:
    _google_api_key = userdata.get("GOOGLE_API_KEY")
except SecretNotFoundError:
    print("Warning: GOOGLE_API_KEY not found in Colab secrets. AI functionalities (Google Gemini) will be disabled. Please set the 'GOOGLE_API_KEY' secret to enable them.")
...
class Settings:
    ...
    ai_api_key: str = _google_api_key # Use the fetched/defaulted key
    google_model: str = os.getenv("GOOGLE_GEMINI_MODEL", "gemini-2.5-flash")
...
def open_app(name: str) -> str:
    # Define cloud-based alternatives for common apps
    cloud_alternatives = {
        "chrome": "https://www.google.com/chrome",
        "edge": "https://www.microsoft.com/edge",
        "firefox": "https://www.mozilla.org/firefox",
        "notepad": "https://docs.new", # Google Docs as a notepad alternative
        "excel": "https://sheets.new", # Google Sheets
        "word": "https://docs.new", # Google Docs
        "powerpoint": "https://slides.new", # Google Slides
        "outlook": "https://outlook.live.com",
        "teams": "https://teams.microsoft.com",
        "zoom": "https://zoom.us",
        "spotify": "https://open.spotify.com",
        "calculator": "https://www.google.com/search?q=calculator", # Google Calculator
    }

    app_lower = name.lower()

    if app_lower in cloud_alternatives:
        url = cloud_alternatives[app_lower]
        webbrowser.open(url)
        return f"Opening {name} via its cloud alternative: {url}."

    # For local applications that don't have a direct cloud alternative
    return f"I cannot open local application '{name}' directly in this Colab environment. You can try searching for a cloud-based version or alternative if available."
...
class CommandRouter:
    ...
    def handle(self, raw_text: str) -> str:
        ...
        if normalized in {"hello", "hi", "hey","jarvis"}:
        ...
        if normalized.startswith("open ") and len(text.split()) >= 2:
            target = text[5:].strip()
            # Check if it's a known website or an app that can be redirected
            if "." in target or target.lower() in {"youtube", "google", "github", "gmail", "chrome", "edge", "firefox", "notepad", "excel", "word", "powerpoint", "outlook", "teams", "zoom", "spotify", "calculator"}:
                # Call open_app first to handle the new logic for apps
                # If open_app suggests a website, open it. Otherwise, handle as before.
                if "." in target:
